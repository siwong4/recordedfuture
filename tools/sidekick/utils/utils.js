import { readBlockConfig, toCamelCase } from './dom.js';
 ...
 */
export function getPageMetadata(elem) {
  const config = {};
  const pageMeta = elem.querySelector('div.page-metadata');
  if (pageMeta) {
    pageMeta.querySelectorAll(':scope > div').forEach((row) => {
      if (row.children) {
        const cols = [...row.children];
        if (cols[1]) {
          const name = toClassName(cols[0].textContent);
...
export function renderScaffolding() {
  return createElement(
    ...
    createElement(
      ...
      [createElement('div', 'menu', {}, [createElement('div', 'list-container')]), createElement('div', 'content')]
    )
  );
}
...
export function processMarkup(pageBlock, path) {
  ...
  copy.querySelector('div.page-metadata')?.remove();
  ...
  try {
    ...
    container.dispatchEvent(
      new CustomEvent('Toast', {
        detail: { message: err.message, variant: 'negative' },
      })
    );
  }
}
      ...
      [
        ...
        createElement('div', 'details-container', {}, [
          createElement('div', 'action-bar', {}, [
            ...
            createElement('div', 'actions', {}, createElement('sp-button', 'copy-button', {}, 'Copy Template')),
          ]),
          createElement('sp-divider', '', { size: 's' }),
