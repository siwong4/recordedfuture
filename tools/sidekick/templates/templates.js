...
async function buildMetadataTable(path) {
  ...
  const pageResp = await fetchBlockPage(path);
  if (pageResp) {
    const metadata = getPageMetadata(pageResp.body);

    const metadataBlock = pageResp.body.querySelector('.page-metadata');
    if (metadataBlock) {
      metadataBlock.remove();
    }

    Object.keys(metadata).forEach((key) => {
      const tableRow = createElement('tr', '', {}, createElement('td', '', {}, key));
      const valTd = createElement('td', '', {});

      valTd.textContent = metadata[key];
...
async function loadTemplates(data, container, sideNav) {
  const promises = data.map(async (item) => {
    ...
    try {
      ...
      const templateNavItem = createElement(
        'sp-sidenav-item',
        '',
        {
          label: name,
        ...
        [
          createElement('sp-icon-file-template', '', {
            slot: 'icon',
            size: 's',
          }),
      ...
      templateNavItem.addEventListener(
        'OnAction',
        () => {
...
export async function decorate(container, data) {
  ...
  container.addEventListener('LoadTemplate', (e) => {
    const { path, page, blockInfo, mdTable } = e.detail;
