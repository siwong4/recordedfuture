...
function optimizeAlgoliaImg(src, alt = '', eager = false) {
  const picture = document.createElement('picture');
  const img = document.createElement('img');
  img.src = src;
  img.alt = alt || '';
...
/**
 * Replace h1 with h2 in default content
 * @param {Element} [element] Element containing headings
 */
function decorateDefaultContentHeading(element) {
  const h1s = [...element.querySelectorAll('.default-content-wrapper h1')];
  h1s.forEach((h1) => {
    const h2 = document.createElement('h2');
    h2.setAttribute('id', h1.getAttribute('id'));
